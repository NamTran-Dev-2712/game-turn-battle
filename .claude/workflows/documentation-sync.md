...
| Changes the config schema | `shared/config-schema/*.schema.json` (per-type + `common.schema.json` + `config-bundle.schema.json`) + matching `fixtures/*.{valid,invalid}.json` + `docs/liveops/remote-config.md` + `docs/gameplay/configuration-and-data.md` (schema-mapping table) + `docs/conventions/data-and-docs-conventions.md` (ID-prefix table) + `config/*/README.md` if a domain is affected. **Breaking change** (remove/rename field, tighten type, narrow enum, change meaning) → bump `schema_version` + add migration under `shared/config-schema/_versions/` (see its README). Never hardcode balance in a schema. Cross-file referential integrity is validated by the phase-07 tool, not JSON Schema alone. |
