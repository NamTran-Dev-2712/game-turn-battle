...
| ~~Golden vector combat (client == server)~~ ✅ **XONG (Phase 26)** — 9 vector đa kịch bản ở `shared/combat-vectors/`; baseline **sinh từ sim server** bằng `tools/combat-baseline` (KHÔNG fork sim); test hai phía **tự khám phá** vector (server `[MemberData]`; client `list_vector_files()`); gate `golden-vector` BLOCKING (`ci-server.yml` = `run.sh check` + golden tests; `ci-client.yml` + trigger `shared/combat-vectors/**`) ⇒ **server ≡ client ≡ baseline**; negative đã kiểm hai phía. **ĐÓNG Nhóm 5 Combat Core.** (Prerequisite Phase 25 client sim đóng cùng đợt.) CI-Actions pending | `shared/combat-vectors`, `tools/combat-baseline`, sim 2 phía | Gameplay (P2) |
| DI thật: ~~EF/Npgsql DbContext~~ ✅ **XONG (Phase 11)** — `AppDbContext`+`UnitOfWork`+`EfRepository`+domain-event dispatch tại SaveChanges + migration `Initial` (schema_metadata seed) + Testcontainers integration; ~~Redis~~ ✅ **XONG (Phase 12)** — `RedisCacheService : ICacheService` (StackExchange.Redis, graceful degradation, key `{env}:{domain}:{name}:{configVersion?}`, `IConnectionMultiplexer` singleton) + `/health` ping Redis + Testcontainers Redis (CachingBehavior chạy thật); ~~JWT~~ ✅ **XONG (Phase 18)**; ~~Configuration Service~~ ✅ **XONG (Phase 21)** — `RuntimeConfigProvider : IConfigProvider` + pipeline `config/→validate(tái dùng validator P07)→bundle bất biến config@vN(checksum)→config_bundles+config_current→cache Redis→flip current nguyên tử`; publish khi deploy (`ConfigPublishHostedService`); endpoint public `/api/v1/config/current`+`/config/bundle`; **ĐÓNG Core Framework P1**; còn lại jobs | `GameTeam.Infrastructure` | P1 |
