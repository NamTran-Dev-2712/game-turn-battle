...
| Repository | `Persistence/Repositories/OwnedHeroRepository.cs` | Hiện thực `IOwnedHeroRepository` (`GetById`+`Add`+**`GetByProfileIdAsync`**+**`GetByProfileAndHeroForUpdateAsync`** — SELECT … FOR UPDATE, Phase 35, tuần tự hoá nâng cấp đồng thời); không rò `IQueryable`/`DbContext`. |
...
- **Seed tạm (tới phase 33):** guest login cấp hero từ config **cùng transaction** account+profile (`CreateGuestAccountCommandHandler`). (Đã gỡ ở Phase 33 — nhận hero qua summon.)
- **Nâng cấp Level (Phase 35):** `OwnedHero.LevelUp()` tăng cột `level` sẵn có (raise `OwnedHeroLeveledUp`) — **không thêm cột ⇒ không cần migration mới** (`has-pending-model-changes` sạch). `LevelUpHeroCommand` khoá dòng hero (`GetByProfileAndHeroForUpdateAsync`) + tiêu gold qua `CurrencyWalletService` (§1.5) trong một transaction. Chỉ số/Power **không lưu** — tính từ config + cấp (`HeroStatCalculator`). Chi tiết: `../gameplay/hero-system.md` §9.
- Test: Testcontainers pg16 — round-trip theo `profile_id`, unique `(profile_id, hero_id)`, dispatch `OwnedHeroGranted` (§5.1); nâng cấp end-to-end + combat theo cấp ở `Api.IntegrationTests/LevelUpHeroEndpointTests` (Phase 35).
