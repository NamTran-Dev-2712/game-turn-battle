...
| economy | `economy.schema.json` | `progression-and-economy.md` | `cost_curves`, `energy` | bước đường cong integer, không cố định; **Phase 35 (additive, không bump version):** optional `level_stat_growth_bp` (tăng trưởng chỉ số/cấp, bp của chỉ số nền) + `power_weights{hp,atk,def,spd}` (trọng số Power Rating); `cost_curves.level_up[i]` = gold lên từ cấp (i+1)→(i+2) ⇒ cấp tối đa = 1 + độ dài mảng. Dữ liệu `config/economy/economy_default.json` |
