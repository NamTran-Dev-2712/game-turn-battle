...
| Đường ghi từ server | `apply_snapshot(snapshot)` — thay **toàn bộ** cache. `apply_wallet(balances)` (Phase 31) — thay **riêng** số dư ví. `apply_inventory(items, owned_heroes)` (Phase 32) — thay **riêng** kho đồ. `apply_campaign_progress(progress)` (Phase 34) — thay **riêng** tiến độ campaign. `apply_heroes(heroes)` (Phase 35) — thay **riêng** danh sách hero sở hữu từ `GET /api/v1/heroes` (`[{id,level,stars}]`, giữ phần khác; dùng sau khi nâng cấp hero). Cả năm đều là **phản chiếu server response**, KHÔNG phải mutator chân lý — **KHÔNG** có `add_currency`/`spend_currency`/`set_currency`/`set_progress`/`add_item`/`set_stage_cleared`/`level_up_hero`… (client không phải chân lý kinh tế/tài sản/tiến độ/cấp). |
| API đọc | `get_currency(code)`, `get_currencies()`, `get_heroes()`, `get_hero(id)`, `get_progress(key)`, `get_all_progress()`, `get_profile()`, `get_inventory()`, `get_campaign_progress()` — **trả BẢN SAO** ⇒ caller không sửa được cache. Số dư ví + số lượng kho + trạng thái mở/khoá/đã-clear campaign là **server-authoritative** — client chỉ hiển thị. |
...
| Sự kiện | Phát `state_refreshed` (§3.1) sau mỗi `apply_snapshot`/`apply_wallet`/`apply_inventory`/`apply_campaign_progress`/`apply_heroes`. |
