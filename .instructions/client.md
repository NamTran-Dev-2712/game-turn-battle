...
- Combat sim: pure/node-decoupled, integer/fixed-point, seeded RNG; match server golden vector (ADR-011). **Golden gate (Phase 26/28):** `client/tests/combat/golden_vector_test.gd` **auto-discovers** every `shared/combat-vectors/*.json` (`CombatVectorLoader.list_vector_files()`, 14 vectors) and matches each against the **server-generated** baseline; client replays, never redefines the result. Baseline change = server-side `tools/combat-baseline` only (never hand-edit a vector). `ci-client.yml` triggers on `shared/combat-vectors/**`.
- **Team & Formation (Phase 29, closed):** feature `src/ui/formation/` (`FormationView` **network-free** + `FormationPresenter`). Presenter reads grid rows×cols from `ConfigProvider.get_entry(&"formation","formation_default")` (data-driven — NO hardcoded grid) + roster from `StateCache.get_heroes()`; keeps a **local draft** (select_hero → place_slot → change position), and on save POSTs the **intent** via `NetworkClient.post_json("/team", …)` then re-renders from the **server-returned team** (never a local save assumed accepted — ADR-007/011); opening the screen loads the saved team via `get_json("/team")`. Parser `NetworkResponseParser.parse_team` → generated `TeamDto`/`TeamSlotDto` (DO-NOT-EDIT). Hub button "Đội hình" (`MainHubView.FEATURES` + `MainHubPresenter` `FORMATION_PATH`). **Reuse** `NetworkClient`/`ConfigProvider`/`StateCache`; **never** a view calling the net, a second HTTP/config/state path, or client-side validation of ownership/count. Canon: `docs/gameplay/hero-system.md` §8 + `.memory/0027`.
- **Skill framework (Phase 28, closed):** `client/src/combat/effects/*` mirrors the server registry (`create_default` = damage/heal/apply_buff/apply_debuff + `stat_modifier_core.gd`); `heal` emits `Healed`, buff/debuff = flat `atk/def/spd` modifier + `duration` (refresh-on-reapply), energy-ultimate (§15) config-gated default-OFF. Must stay **bit-for-bit** with the server (golden). Extend effect = 1 handler mirroring server + config; **never** fork a second client sim, use `float`/global RNG, or `switch(effect_type)`. New vector skill format (`config_excerpt.skills` + `team_snapshot[].skills`) lives in `client/tests/combat/support/combat_vector_loader.gd`. Canon: `docs/gameplay/skill-framework.md`.
- **Battle flow (Phase 30, closed — ĐÓNG P2):** feature `src/ui/battle/` (`BattleView` **network-free** + `BattlePresenter`). Vào từ hub nút "Chiến đấu" (`MainHubPresenter` `BATTLE_PATH` với `{stage_id}`; chọn stage = phase 34). Presenter: `get_json("/team")` → `post_json("/battles", {teamId, stageId, attemptId})` → nhận `BattleResultDto` → **replay** bằng seed server (client `CombatInputResolver` + `BattleSimulator` + `ConfigProvider`) để vẽ diễn biến. **Server là authority (ADR-011/007):** `outcome`/`rewards` HIỂN THỊ theo server; replay lệch ⇒ hiện theo server + `push_warning` (NEVER fabricate); `attemptId` (idempotency key) do client sinh mỗi lần đánh (re-fight = trận mới; server chống double-grant); client KHÔNG chọn seed / KHÔNG tự cấp thưởng. Parser `parse_battle_result` → generated `BattleResultDto`/`RewardDto`; `parse_team` đọc `id` (client gửi teamId). **Client resolver (`src/combat/combat_input_resolver.gd`) đọc gameplay config THẬT** (base_stats+skills[]; skill target/trigger/effects[].params.coeff_fixed; enemy actor `enemy_{i}`) — **giống hệt server** (replay bit-identical); never fork a second resolver/sim. Ví/currency toàn cục ở hub = phase 31 (Phase 30 chỉ hiện thưởng của trận). Canon: `docs/gameplay/combat-framework.md` §24 + `docs/godot/ui-architecture.md` §4.2 + `.memory/0028`.
- **Campaign PvE (Phase 34, closed):** feature `src/ui/campaign/` (`CampaignView` **network-free** + `CampaignPresenter`). Vào từ hub nút "Chiến dịch" (`MainHubView.FEATURES` + `MainHubPresenter` `CAMPAIGN_PATH`). Presenter: `get_json("/campaign/progress")` (parser `parse_campaign_progress` → generated `CampaignProgressDto`/`CampaignStageDto`) → `StateCache.apply_campaign_progress(...)` (slice mới, display-only, phát `state_refreshed`) → render trạng thái **mở/khoá/đã-clear** (server-authoritative) ghép preview địch/thưởng từ `ConfigProvider.get_entry(&"stage", id)`. Đánh stage = **reuse màn battle** (`SceneRouter.goto_scene(BATTLE_PATH, {"campaign_stage_id": id})`; `BattlePresenter` đọc route context ⇒ POST `/campaign/battles` thay `/battles`); back → scene re-instantiate ⇒ presenter tự fetch lại tiến độ. UX chỉ cho bấm stage **unlocked** (nút `disabled` khi khoá) — **bảo mật không phụ thuộc client** (server vẫn validate anti-skip → 403). Fallback KHÔNG im lặng (Rule E): lỗi tải → giữ cache + banner lỗi + Thử lại. **KHÔNG thêm EventBus event** (reuse `state_refreshed`/`config_updated`). Client là display-only (ADR-007/011): KHÔNG tự suy tiến độ/mở-khoá. **Reuse** `NetworkClient`/`ConfigProvider`/`StateCache`/`SceneRouter` + màn battle; **never** view gọi net, fork HTTP/state path thứ 2, hay client quyết tiến độ. Verify: gdUnit4 156 (`campaign_presenter_test.gd`: render locked/unlocked/cleared, open unlocked→battle context, open locked→no-nav, fetch fail→giữ cache, newly-unlocked sau update, back). Canon: `docs/gameplay/progression-and-economy.md` §2b + `.memory/0032`.
