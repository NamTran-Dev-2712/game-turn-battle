...
- **Formation config (Phase 29):** loại config MỚI `formation` — `config/formation/formation_default.json` (schema `formation.schema.json`): `rows`/`cols` (integer ≥1) ⇒ **team size = rows×cols** (KHÔNG bonus vị trí — tuning/Post-MVP). Thêm loại config = hợp đồng Phase 06/07: `ConfigType.Formation` + `ConfigFileMapper["formation"]` + `common.schema.json#/$defs/formation_id` + fixtures (`formation.valid|invalid.json`) + mapper test — `ConfigType` enum điều khiển CẢ validator gate LẪN `ConfigBundleBuilder` (Enum.GetValues). Đọc runtime qua `IConfigProvider.Get<FormationConfig>("formation","formation_default")` (KHÔNG hardcode grid). Canon: `docs/gameplay/hero-system.md` §8 + `.memory/0027`.
- **Item catalog config (Phase 32):** loại config MỚI `item` — `config/items/*.json` (schema `item.schema.json`): `item_id` (prefix `item_`), `item_type` (enum, MVP `item`), `name` tuỳ chọn — **KHÔNG balance/số lượng** (số lượng thuộc kho người chơi, server-authoritative). Thêm loại config = hợp đồng Phase 06/07: `ConfigType.Item` + `ConfigFileMapper["items"]` + `common.schema.json#/$defs/item_id`+`item_type` + fixtures (`item.valid|invalid.json`) + mapper test. **Fragment KHÔNG phải catalog item** — mảnh của hero (tham chiếu `hero_id`); reward ref đa hình nay kiểm tồn tại: `item`→item index, `fragment`→hero index (`ReferenceValidator.Reward`). Inventory (server) kiểm id qua `IConfigProvider.GetIds("item")`/`GetIds("hero")`. Canon: `docs/gameplay/inventory-and-equipment.md` §1 + `docs/gameplay/configuration-and-data.md` §2b + `.memory/0030`.
- **Skill config (Phase 28):** `config/skills/*.json` (schema `skill.schema.json`) = `target` + `trigger`(energy/cooldown) + optional `cooldown` + `effects[]` (`{effect_type, target?, params}`). `params` typed tuỳ chọn: `coeff_fixed`/`amount_fixed`/`atk`/`def`/`spd`/`duration` (đều `combat_int`) — **additive, KHÔNG bump `schema_version`**; giá trị là tuning (KHÔNG balance ở schema). `effect_type` ∈ damage/heal/apply_buff/apply_debuff/shield (thêm loại = enum additive + handler 2 phía). Combat runtime đọc lát cắt `SkillCombatConfig` (Application). **Phase 30 nối `config/skills`→battle thật** qua `hero.skills[]` (basic = skill energy==0, ultimate = skill energy>0). Canon: `docs/gameplay/skill-framework.md`.
- **Stage + reward config (Phase 30):** `config/stages/*.json` (schema `stage.schema.json`) — Phase 30 mở rộng **additive** (KHÔNG bump `schema_version`): optional `max_rounds`/`basic_skill_id`/`combat_rules{def_constant_k,min_damage,crit_multiplier_fixed,accuracy_bp,crit_rate_bp,energy{…}}` + enemy `slot` để nối sim thật; `enemies[].hero_id`→hero, `rewards[]`→reward (referential integrity ở validator). `config/rewards/*.json` (`reward.schema.json`) = `entries[{reward_type,ref_id,amount}]`; Phase 30 cấp **chỉ `currency`** (ref ∈ gold/gem/ticket) khi VICTORY → credit ví server-side (hero/fragment/item = phase 31–33). Demo: `config/stages/stage_demo_01.json` + `config/rewards/reward_stage_demo.json` + 4 hero đủ đội 6. `combat_rules` giá trị = tuning (KHÔNG balance ở schema). Canon: `docs/gameplay/combat-framework.md` §24 + `.memory/0028`.
- **Chapter config (Phase 34):** loại config MỚI `chapter` — `config/chapters/*.json` (schema `chapter.schema.json`): `chapter_id` (prefix `chapter_`), `order` (integer, thứ tự chương), `stages[]` (id stage theo thứ tự chơi, unique), `name` tuỳ chọn — **KHÔNG balance** (địch/thưởng/độ khó ở stage). Chuỗi campaign = các chapter theo `order` (tie-break id) rồi `stages` theo thứ tự ⇒ mở khoá **tuần tự** (chống skip). Thêm loại config = hợp đồng Phase 06/07: `ConfigType.Chapter` + `ConfigFileMapper["chapters"]` + `common.schema.json#/$defs/chapter_id` + `ReferenceValidator.Chapter` (`stages[]`→stage index) + fixtures (`chapter.valid|invalid.json`) + mapper/reference test. Data campaign mẫu: `config/chapters/chapter_01.json` + `config/stages/stage_ch01_0{1,2,3}.json` + `config/rewards/reward_ch01_0{1,2,3}.json` (reuse hero/skill sẵn có). Đọc runtime qua `IConfigProvider.Get<ChapterConfig>("chapter",id)`/`GetIds("chapter")` (server `CampaignChain`). Thêm chapter/stage mới = **chỉ config**, không sửa business logic. Canon: `docs/gameplay/progression-and-economy.md` §2b + `docs/gameplay/configuration-and-data.md` §2b + `.memory/0032`.
