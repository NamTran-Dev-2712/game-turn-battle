...
/// <summary>
/// Handles <see cref="RemoveItemsCommand"/> — handler <b>mỏng</b>: suy chủ sở hữu từ token
/// (<see cref="ICurrentUser"/>) → profile, rồi uỷ cho cơ chế dùng chung <see cref="InventoryService"/>
/// (idempotency + kiểm config + khoá dòng + bớt atomic + ledger). Bao trong transaction bởi <c>TransactionBehavior</c>.
/// </summary>
public sealed class RemoveItemsCommandHandler
    : IRequestHandler<RemoveItemsCommand, Result<InventoryTransactionResult>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _inventory.ConsumeAsync(
