...
/// <summary>
/// Handles <see cref="GrantCurrencyCommand"/> — handler <b>mỏng</b>: suy chủ sở hữu từ token
/// (<see cref="ICurrentUser"/>) → profile, rồi uỷ cho cơ chế dùng chung <see cref="CurrencyWalletService"/>
/// (idempotency + khoá dòng + credit + ledger). Bao trong transaction bởi <c>TransactionBehavior</c>.
/// </summary>
public sealed class GrantCurrencyCommandHandler
    : IRequestHandler<GrantCurrencyCommand, Result<CurrencyTransactionResult>>
{
    private readonly ICurrentUser _currentUser;
    private readonly IPlayerProfileRepository _profiles;
    private readonly CurrencyWalletService _wallet;

    public GrantCurrencyCommandHandler(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _wallet.GrantAsync(
